...
class InPlaceFusionDMT(BaseDMT):
    ...
    def _eval_segment(self, seg, graph, hw_params, mapping, config, direct_mapping_table):
        """评估一个连续子段 seg ，根据能否常驻决定记账模式。返回 (lat, eng, buf_loss, compat_pen)。"""
        from types import SimpleNamespace
        from dosa.performance_model import HighFidelityPerformanceModel
        # 构造仅包含 seg 的伪图
        sub_layers = {name: graph.layers[name] for name in seg}
        pseudo_graph = SimpleNamespace(layers=sub_layers, fusion_groups=[seg])

        perf_model = HighFidelityPerformanceModel(config)

        # 段级常驻检查
        try:
            can_persist = self._can_persist_group_W(seg, graph, hw_params, config, level_name='L2_Scratchpad')
        except Exception:
            can_persist = False
        ...
        lat, eng, _, buf_loss, compat_pen = perf_model.evaluate_group_depth_first(
            group_layers=seg,
            graph=pseudo_graph,
            hw_params=hw_params,
            mapping=mapping,
        ...
        if hasattr(mapping, 'has_partial_sums') and mapping.has_partial_sums():
            compat_pen += torch.tensor(1.0, device=config.DEVICE)

        return lat, eng, buf_loss, compat_pen

    def _greedy_longest_prefix_partition(self, group, graph, hw_params, mapping, config, direct_mapping_table):
        """贪心最长前缀可常驻分段算法，遍历整个融合链并累计各段指标。"""
        total_lat = torch.tensor(0.0, device=config.DEVICE)
        total_eng = torch.tensor(0.0, device=config.DEVICE)
        total_buf = torch.tensor(0.0, device=config.DEVICE)
        total_pen = torch.tensor(0.0, device=config.DEVICE)
        ...
        while i < n:
            ...
            for j in range(n, i, -1):
                seg = group[i:j]
                try:
                    if self._can_persist_group_W(seg, graph, hw_params, config, level_name='L2_Scratchpad'):
                        best_j = j
                        break
                except Exception:
                    continue
            ...
            lat, eng, buf, pen = self._eval_segment(group[i:best_j], graph, hw_params, mapping, config, direct_mapping_table)
            total_lat += lat
            total_eng += eng
            total_buf += buf
            total_pen += pen
        ...
        return total_lat, total_eng, total_buf, total_pen

    def forward(self, group: list, graph: ComputationGraph, hw_params: HardwareParameters, 
                mapping: FineGrainedMapping, config: Config, direct_mapping_table: dict = None) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, dict]:
        
        # === Final-Sum-Per-Tile 开关：默认在DMT入口强制启用 ===
        if hasattr(mapping, 'set_enforce_no_partial_sum'):
            mapping.set_enforce_no_partial_sum(True)
